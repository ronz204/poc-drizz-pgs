...
- If you wrote or edited a `<slice>.plan.md`, confirm its Status section actually matches which steps are done — a stale status is worse than no plan at all. If every step is now done and that's reflected in the code and in `spec.md`/`design.md`, delete the plan file instead of leaving it at "done".
