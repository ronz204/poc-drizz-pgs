...
- Each bounded context also has an `index.ts` at its own root, acting as its single public entry point and the one place outside `contexts/` and `contracts/` allowed to import from either: it re-exports whatever `contexts/` or `contracts/` content outside code actually needs — an aggregate, an entity, a value object, a domain error, an enum, a type, or a port/DTO from `contracts/` — driven by real, current need rather than exporting every symbol those folders happen to contain speculatively. Other code imports the bounded context through `index.ts` — never by reaching into `contracts/`'s own files, or into `contexts/`, directly.
